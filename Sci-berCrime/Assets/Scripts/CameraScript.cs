...
public class CameraScript : MonoBehaviour
{
    ...
    private void Update ()
    {
        ...
        else if (m_goPlayerOne.GetComponent<PlayerController>().IsAlive && !m_goPlayerTwo.GetComponent<PlayerController>().IsAlive)
        {
            ...
            m_goCameraPlane_Pan.transform.position = m_goPlayerOne.transform.position / m_fCameraCenterTether + m_goCameraPlane_Zoom.transform.position;
        }
        else if (!m_goPlayerOne.GetComponent<PlayerController>().IsAlive && m_goPlayerTwo.GetComponent<PlayerController>().IsAlive)
        {
            m_goCameraPlane_Zoom.transform.position = m_goCameraDolly.transform.position + new Vector3(0, m_fZoomOffset);

            m_goCameraPlane_Pan.transform.position = m_goPlayerTwo.transform.position / m_fCameraCenterTether + m_goCameraPlane_Zoom.transform.position;
        }
    }
